...
int part1() {
    ...
    if (!pointsfile.is_open()) {
        cerr << "Unable to open points.txt";
        return -1;
    }
    vector<Point> points;
    double x, y;
...
double crossProductCCW(const Point& O, const Point& A, const Point& B) {
    double val = (A.get_x() - O.get_x()) * (B.get_y() - A.get_y()) - (A.get_y() - O.get_y()) * (B.get_x() - A.get_x());
    if (val==0) { return 0; }
    if (val>0) { return -1; }
    else { return 1; }
}

static bool compareY(const Point& a, const Point& b) { return a.get_y()<b.get_y(); }

static bool comparePolarAngleNoReference(const Point& p1, const Point& p2) {
    int direction=crossProductCCW(minYpoint, p1, p2);
    if (direction==0) { return(distance(minYpoint, p1) < distance(minYpoint, p2)); }
...
vector<Point> grahamScan(vector<Point>& points) {
    stack<Point> grahampoints;
    int pointssize=points.size();
    
    sort(points.begin(), points.end(), compareY);
    minYpoint=points[0];
    sort(points.begin()+1, points.end(), comparePolarAngleNoReference);
    
    grahampoints.push(points[0]);
    grahampoints.push(points[1]);
    
    for (int i=2; i<pointssize; i++) {
        Point temp=points[i];
        Point topPt=grahampoints.top();
        grahampoints.pop();
        Point nextPt=grahampoints.top();
        while (grahampoints.size()>1&&crossProductCCW(nextPt, topPt, temp)<1) {
            topPt=grahampoints.top();
            grahampoints.pop();
            nextPt=grahampoints.top();
        }
        if (crossProductCCW(nextPt, topPt, temp)>0) { grahampoints.push(topPt); }
        grahampoints.push(temp);
    }

    vector<Point> convexpoints;
    while (grahampoints.size() > 0) {
        convexpoints.insert(convexpoints.begin(), grahampoints.top());
        grahampoints.pop();
    }
...
int part2() {
    ...
    auto convexpoints=grahamScan(points);

    // create a ppm of size 400x400 named grahamscan.ppm in which you display a circle of radius 3 for each scaled point you generated, 
    // as well as connect the vertices you obtained for the convex hull (so in the image I should see 60 points and a convex hull with 
    // vertices circles of radius 3 that are conected forming a convex hull, draw the vertices of the convex hull in a red color)
    LineSegment l=LineSegment(0,0,0,0);
    Circle c=Circle(0,0,0);
    Circle c2=Circle(0,0,0);

    Point firstPoint=convexpoints[0];
    Point origPoint=convexpoints[0];
    ...
    for (int i=1; i<(int)(convexpoints.size()); i++) {
        Point curr=convexpoints[i];
